...
uint8_t wState = STATE_LedBattLevel;       // 1=ledGreen, 2=ledOrange, 4=ledRed, 8=ledUp, 16=ledDown   , 32=Battery3Led,
                                           // 64=Disable, 128=ShutOff
uint8_t wStateSlave = STATE_LedBattLevel;  // 1=ledGreen, 2=ledOrange, 4=ledRed, 8=ledUp, 16=ledDown   , 32=Battery3Led,
                                           // 64=Disable, 128=ShutOff
...
const float lookUpTableAngle[181] = {-1,
                                     -0.937202577,
                                     -0.878193767,
                                     -0.822607884,
                                     -0.770124422,
...
int main(void) {
  ...
  SysTick_Config(
      ...
      1000);  //  Configure SysTick to generate an interrupt every millisecond
              // Clock_test();		// 72Mhz: iTestClock=12000, 64Mhz=13500, 48Mhz=18000 = 18 seconds fron power on
              // to startup melody. 124Mhz = 7000
...
#endif
// #ifdef USART1_BAUD
// 	USART1_Init(USART1_BAUD); //TODO: usunąć, w zamian dodać inicjalizację komunikacji
// #endif
#ifdef USART2_BAUD
  ...
  */
  ...
  BUZZER_MelodyDown()
      ...
      // 		#endif
      // 	}
      // 	Delay(10); //debounce to prevent immediate ShutOff (100 is to much with a switch instead of a push
      // button) #endif
  ...
  while (1) {
    if (millis() < iTimeNextLoop) continue;
    ...
    if (MPU_ReadAll() != SUCCESS) {
      ...
      if (plotCounter >= 5) {
        plotCounter = 0;
        char plotStr[64];
        snprintf(plotStr, sizeof(plotStr), "%d,%d,%d,%d,%d,%d,%d\n", mpuData.accel.x, mpuData.accel.y, mpuData.accel.z,
                 mpuData.gyro.x, mpuData.gyro.y, mpuData.gyro.z, mpuData.temperature);
    ...
    // 		{
    // 			// #if defined(PILOT_DEFAULT) && defined(IMU_ENABLE) && (defined(SEND_IMU_DATA) ||
    // defined(RTT_LOG_MPU))
    // 			// 	if (MPU_ReadAll() != SUCCESS)
    // 			// 	{
    // 			// 		I2C_Init();	// try to re-init if i2c bus fails. Need when I2C_SPEED is
    // 400000 instead of 200000
    // 			// 		//MPU_Init();		// re-init mpu6050 (not really needed)
    // 			// 		iBug++;
    // 			// 	}
    ...
    // 			// Each speedvalue or steervalue between 50 and -50 (STAND_STILL_THRESHOLD) means absolutely no
    // pwm
    // 			// -> to get the device calm 'around zero speed'
    // 			scaledSpeed = speed < STAND_STILL_THRESHOLD && speed > -STAND_STILL_THRESHOLD ? 0 : CLAMP(speed,
    // -speedLimit, speedLimit) * SPEED_COEFFICIENT; 			scaledSteer = steer < STAND_STILL_THRESHOLD &&
    // steer > -STAND_STILL_THRESHOLD ? 0 : CLAMP(steer, -speedLimit, speedLimit) * STEER_COEFFICIENT * expo;
    ...
    // 		if (batteryVoltage > BAT_LOW_LVL1) // Show green battery symbol when battery level BAT_LOW_LVL1 is
    // reached
    // 		{
    // 			ShowBatteryState(0);
    ...
    // 			}
    // 		}
    // 		else if (batteryVoltage > BAT_LOW_LVL2) // Make silent sound and show orange battery symbol when battery
    // level BAT_LOW_LVL2 is reached
    // 		{
    // 			ShowBatteryState(1);
    ...
    // #endif
    // 		}
    // 		else if (batteryVoltage > BAT_LOW_DEAD) // Make even more sound and show red battery symbol when battery
    // level BAT_LOW_DEAD is reached
    // 		{
    // 			ShowBatteryState(2);
    // #ifdef BATTERY_LOW_BEEP
...
//TEMPTEMPTEMP
    ...
    //           SEGGER_RTT_WriteString(0, inputBuffer + 1);  // Wypisz to co przyszło
    //           SEGGER_RTT_WriteString(0, "\r\n");
...
int32_t ShutOff(void) {
  if (speedShutoff) {
    ...
    if (ABS(speedShutoff) > 10) return speedShutoff;
...
#  endif
  ...
  // TARGET_nvic_irq_disable(USART1_IRQn); //TODO: a może to dodać
  // TODO: Dodać to:
  // Fragment w ProessReceived po stronie SLAVE:
  // if (pData->flags & STATE_Shutoff) {
  //     ShutOff(); // Slave też się wyłącza, gdy dostanie flagę od Mastera
  // }
...
#  ifdef DEBUG_LED
  return;
#  else
  if (!(wState & STATE_LedBattLevel)) return;
  uint32_t aPin[3];
  uint8_t iLedCount = 0;
